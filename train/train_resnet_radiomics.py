...
from networks.resnet_radiomics import resnet18, resnet34, resnet50
...
parser.add_argument('--ada_loss', action='store_true', help='adaptive contrastive loss')
...
parser.add_argument("--multi_tasks", action="store_true", help="use multi-tasks")
...
def main():
    ...
    if args.model_name == "resnet_18":
        model = resnet18(spatial_dims=3, n_input_channels=1, num_classes=2, num_radiomics_features=args.num_radiomics_features, fused_pattern=args.fused_pattern, ada_loss=args.ada_loss).cuda(args.gpu)
    elif args.model_name == "resnet_34":
        model = resnet34(spatial_dims=3, n_input_channels=1, num_classes=2, num_radiomics_features=args.num_radiomics_features, fused_pattern=args.fused_pattern, ada_loss=args.ada_loss).cuda(args.gpu)
    elif args.model_name == "resnet_50":
        model = resnet50(spatial_dims=3, n_input_channels=1, num_classes=2, num_radiomics_features=args.num_radiomics_features, fused_pattern=args.fused_pattern, ada_loss=args.ada_loss).cuda(args.gpu)
    else:
    ...
    loss_function_aux_ce = torch.nn.CrossEntropyLoss(ignore_index=-1)
    loss_function_aux_mse = torch.nn.MSELoss()
    ...
    for epoch in range(args.start_epoch, args.max_epochs):
        ...
        for step, batch_data in enumerate(train_loader):
            ...
            if args.cont_loss:
                ...
                if args.ada_loss:
                    train_loss = torch.exp(-s1) * loss_function(outputs, labels) + s1 + torch.exp(-s2) * cont_loss + s2
                else:
            ...
            if args.multi_tasks:
                train_loss = train_loss + loss_function_aux_ce(outputs_1, labels_int_lvi) + loss_function_aux_ce(outputs_2, labels_int_pgrade) + loss_function_aux_ce(outputs_3, labels_int_ptype) + loss_function_aux_mse(outputs_4, labels_float_solid_component.float())
        ...
        if (epoch + 1) % args.val_every == 0:
            ...
            with torch.no_grad():
                ...
                for val_data in tqdm(val_loader):
                    val_data["radiomics"] = torch.stack(val_data["radiomics"]).transpose(0, 1).float()
                    val_images, val_labels, val_radiomics_features = val_data["image"].cuda(args.gpu), val_data["label"].cuda(args.gpu), val_data["radiomics"].cuda(args.gpu)
                    y_pred = torch.cat([y_pred, model(val_images, val_radiomics_features)[0]], dim=0)
                    y_pred_ema = torch.cat([y_pred_ema, ema.module(val_images, val_radiomics_features)[0]], dim=0)
