...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        num_radiomics_features: int = 10,
        fused_pattern: str = "add",
        ada_loss: bool = False,
    ) -> None:
        ...
        self.fused_pattern = fused_pattern
        ...
        if self.fused_pattern == "cat_only":
            num_fused_features = num_radiomics_features + block_inplanes[3] * block.expansion
        elif self.fused_pattern == "cat":
            self.radiomics_fc = nn.Linear(num_radiomics_features, block_inplanes[3] * block.expansion)
            num_fused_features = block_inplanes[3] * block.expansion * 2
        elif self.fused_pattern == "add":
            self.radiomics_fc = nn.Linear(num_radiomics_features, block_inplanes[3] * block.expansion)
            num_fused_features = block_inplanes[3] * block.expansion
        ...
        self.fc = nn.Linear(num_fused_features, num_classes) if feed_forward else None
        self.fc1 = nn.Linear(num_fused_features, num_classes) if feed_forward else None
        self.fc2 = nn.Linear(num_fused_features, num_classes) if feed_forward else None
        self.fc3 = nn.Linear(num_fused_features, num_classes) if feed_forward else None
        self.fc4 = nn.Linear(num_fused_features, 1) if feed_forward else None
    ...
    def forward(self, x: torch.Tensor, radiomics_features: torch.Tensor) -> torch.Tensor:
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        # print(x.shape)  # torch.Size([12, 512/2048, 6, 6, 6])

        x = self.avgpool(x)
        # print(x.shape)  # torch.Size([12, 512/2048, 1, 1, 1])

        x = x.view(x.size(0), -1)
        # print(x.shape)  # torch.Size([12, 512/2048])
        ...
        if self.fc is not None:
            ...
            # print(logits.shape)  # torch.Size([12, 2])
            ...
            # print(logits4.shape)
            if self.contrastive_features is not None:
                cont_features = self.contrastive_features(fused_features)
            # print(cont_features.shape)  # torch.Size([12, 128])
