...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        if self.fused_pattern == "cat_only":
            num_fused_features = num_radiomics_features + in_channels
        elif self.fused_pattern == "cat":
            self.radiomics_fc = nn.Linear(num_radiomics_features, in_channels)
            num_fused_features = in_channels * 2
        elif self.fused_pattern == "add":
            self.radiomics_fc = nn.Linear(num_radiomics_features, in_channels)
            num_fused_features = in_channels
        ...
        self.class_layers = nn.Sequential(
            OrderedDict(
                [
                    ("out", nn.Linear(num_fused_features, out_channels)),
                ]
            )
        )
        self.fc1 = nn.Linear(num_fused_features, out_channels)
        self.fc2 = nn.Linear(num_fused_features, out_channels)
        self.fc3 = nn.Linear(num_fused_features, out_channels)
